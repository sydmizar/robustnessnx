...
#    np_clustering = np.true_divide(np_clustering, i)
    ...
    df.to_csv('np_avg_degree2_'+str(type_proj)+'_'+str(type_method)+'.csv', index=True, header=True, sep=',', encoding = 'utf-8-sig')
    
    df = pd.DataFrame(np_conn_nodes, index=p_vector, columns=p_vector)
    df.to_csv('np_conn_nodes2_'+str(type_proj)+'_'+str(type_method)+'.csv', index=True, header=True, sep=',', encoding = 'utf-8-sig')
    
    df = pd.DataFrame(np_unconn_nodes, index=p_vector, columns=p_vector)
    df.to_csv('np_unconn_nodes2_'+str(type_proj)+'_'+str(type_method)+'.csv', index=True, header=True, sep=',', encoding = 'utf-8-sig')
    
    df = pd.DataFrame(np_conn_components, index=p_vector, columns=p_vector)
    df.to_csv('np_conn_components2_'+str(type_proj)+'_'+str(type_method)+'.csv', index=True, header=True, sep=',', encoding = 'utf-8-sig')
    
    df = pd.DataFrame(np_mean_size, index=p_vector, columns=p_vector)
    df.to_csv('np_mean_size2_'+str(type_proj)+'_'+str(type_method)+'.csv', index=True, header=True, sep=',', encoding = 'utf-8-sig')
